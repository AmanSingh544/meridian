        ...
        {MODE_OPTIONS.map((opt) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer', minWidth: '6.5rem',
              transition: 'border-color 0.15s, background 0.15s',
            }}
          >
            <span style={{ fontSize: '1.375rem' }}>{opt.icon}</span>
            <div>
              <div style={{ fontSize: '0.8125rem', fontWeight: colorMode === opt.value ? 600 : 400, color: colorMode === opt.value ? 'var(--color-brand-700)' : 'var(--color-text)', textAlign: 'center' }}>
                {opt.label}
              </div>
              <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', textAlign: 'center', marginTop: '0.125rem' }}>
                {opt.hint}
              </div>
            </div>
            {colorMode === opt.value && (
      ...
      <div style={{ display: 'flex', gap: '0.875rem', marginBottom: '1.5rem' }}>
        {(['comfortable', 'compact'] as const).map((d) => (
          ...
          >
            ...
            <span style={{ fontSize: '0.8125rem', fontWeight: density === d ? 600 : 400, color: density === d ? 'var(--color-brand-700)' : 'var(--color-text)', textTransform: 'capitalize' }}>
              {d}
            </span>
          </button>
        ))}
...
const NotificationsTab: React.FC = () => {
  ...
  useEffect(() => {
    if (!prefs) return;
    setTicketAssigned(prefs.emailOnTicketAssigned ?? true);
    setTicketReplied(prefs.emailOnNewReply ?? true);
    setSlaWarning(prefs.emailOnSLAWarning ?? true);
    setEscalationAlert(prefs.emailOnEscalation ?? true);
    setDailyDigest(prefs.emailDailyDigest ?? false);
  }, [prefs]);
  ...
  return (
    <div>
      <SectionHeader title="In-App & Email Notifications" description="Choose which events trigger notifications." />
      <NotifRow label="Ticket assigned to me" hint="When a ticket is assigned to your queue" value={ticketAssigned} onChange={setTicketAssigned} />
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: '22rem', display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
        <div>
          <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.375rem' }}>Current password</label>
        ...
        <div>
          <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.375rem' }}>Confirm new password</label>
    ...
    <div style={{ maxWidth: '100%', margin: '0 auto' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ margin: '0.25rem 0 0', fontSize: '0.875rem', color: 'var(--color-text-secondary)' }}>
          Manage your profile, appearance preferences, and security.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '12rem 1fr', gap: '1.5rem', alignItems: 'start' }}>
        {/* Sidebar */}
        <div>
          <Card padding="0.5rem">
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  ...
                  background: activeTab === tab.id ? 'var(--color-brand-600)' : 'transparent',
          ...
          <Card hover padding="0.875rem" style={{ marginTop: '1rem', background: 'var(--color-bg-subtle)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', textAlign: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', textTransform: 'capitalize', marginTop: '0.125rem' }}>
